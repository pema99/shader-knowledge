...
It is slightly misleading to say that only floating point texture formats need double buffering. Actually, all formats do if the HDR rendering isn't for the cameras involved. When HDR is enabled, the camera will blit to a 16-bit floating point texture format under the hood. When an integer texture format is used, this mismatch in formats essentially implements double buffering for you. A working non double buffered camera loop using a integer format will break if HDR is disabled.
